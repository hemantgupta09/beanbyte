...
var PB_RA_statement = function () {
  ...
  const navtab_stmt_charts = function (element, button) {
    ...
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL([], {
      act : pb.rpt.accounts.statement.p.charts,
      type: ''
    });
    ajax.callREQUEST({thePathArr}, urlID, false);
    const responseX = ajax.getRESPONSE();
    ...
    const debited = function (element, event) {
      ...
      if (!chartObject[pb.rpt.accounts.statement.c.chart.debited_account]) {
        ...
        chartObject[pb.rpt.accounts.statement.c.chart.debited_account] = Plug_apexChart.Init_onCalling(targetEle, redData);
      }
      else {
        // data loading into chart
        Plug_apexChart.Update_onCalling(chartObject[pb.rpt.accounts.statement.c.chart.debited_account], redData);
      }
    ...
    const credited = function (element, event) {
      ...
      if (!chartObject[pb.rpt.accounts.statement.c.chart.credited_account]) {
        ...
        chartObject[pb.rpt.accounts.statement.c.chart.credited_account] = Plug_apexChart.Init_onCalling(targetEle, redData);
      }
      else {
        // data loading into chart
        Plug_apexChart.Update_onCalling(chartObject[pb.rpt.accounts.statement.c.chart.credited_account], redData);
      }
    }
    ...
    const balance = function (element, event) {
      ...
      if (!chartObject[pb.rpt.accounts.statement.c.chart.balance]) {
        ...
        chartObject[pb.rpt.accounts.statement.c.chart.balance] = Plug_apexChart.Init_onCalling(targetEle, redData);
      }
      else {
        // data loading into chart
        Plug_apexChart.Update_onCalling(chartObject[pb.rpt.accounts.statement.c.chart.balance], redData);
      }
    ...
    const transaction = function (element, event) {
      ...
      if (!chartObject[pb.rpt.accounts.statement.c.chart.transaction]) {
        ...
        chartObject[pb.rpt.accounts.statement.c.chart.transaction] = Plug_apexChart.Init_onCalling(targetEle, redData);
      }
      else {
        // data loading into chart
        Plug_apexChart.Update_onCalling(chartObject[pb.rpt.accounts.statement.c.chart.transaction], redData);
      }
    }
    /*
