...
var PB_RP_rates = function () {
  ...
  const report_rate_rates = function (element, event) {
  ...
  const report_rate_variation = function (element, event) {
  ...
  const report_rate_stock = function (element, event) {
  ...
  const tab_rate_rates = function (element, data) {
  ...
  const tab_rate_variation = function (element, data, event) {
  ...
  const tab_rate_stock = function (element, data, event) {

    // value putting for category_id into petroleum.
    redData = PB_extend_foreign.$_remote('single', data, [['vehicles', 'petroleum', 2, 1]]);
  ...
  const tabsTarget = function (element, event) {
    ...
    const _process_a = (data) => {
      console.log(data);
      switch (event.value) {
        case pb.rpt.petroleum.rates.p.rates:
          return tab_rate_rates(element, data, event);

        case pb.rpt.petroleum.rates.p.variation:
          return tab_rate_variation(element, data, event);

        case pb.rpt.petroleum.rates.p.stock:
          return tab_rate_stock(element, data, event);
      }
    }
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL(thePathArr, {
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.petroleum.rates.p.rates:
        return report_rate_rates(element, event);
      case pb.rpt.petroleum.rates.p.variation:
        return report_rate_variation(element, event);
      case pb.rpt.petroleum.rates.p.stock:
        return report_rate_stock(element, event);
