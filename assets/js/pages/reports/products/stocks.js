...
var PB_RP_stocks = function () {
  ...
  const report_stock_item = function (element, event) {
  ...
  const report_stock_location = function (element, event) {
  ...
  const tab_stock_item = function (element, data, event) {
  ...
  const tab_stock_location = function (element, data, event) {
    ...
    const _process_a = (data) => {
      ...
      switch (event.value) {
        case pb.rpt.products.stocks.p.item:
          return tab_stock_item(element, data, event);

        case pb.rpt.products.stocks.p.location:
          return tab_stock_location(element, data, event);
      }
    }
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL(thePathArr, {
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.products.stocks.p.item:
        return report_stock_item(element, event);

      case pb.rpt.products.stocks.p.location:
        return report_stock_location(element, event);
