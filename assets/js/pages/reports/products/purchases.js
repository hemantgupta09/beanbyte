...
var PB_RP_purchases = function () {
  ...
  const report_purchase_category = function (element, event) {
  ...
  const report_purchase_supplier = function (element, event) {
  ...
  const tab_purchase_category = function (element, data, event) {
  ...
  const tab_purchase_supplier = function (element, data, event) {
    // value putting for category_id into customers.
    redData = PB_extend_foreign.$_remote('single', data, [['customers', 'nicknames', 2, 1]]);
    // append options into selects.
    PB_render_form.renderME(rendER.form.method.selectOnly, redData, element, atr.core.source);
    ...
    const _process_a = (data) => {
      ...
      switch (event.value) {
        case pb.rpt.products.purchases.p.category:
          return tab_purchase_category(element, data, event);
        ...
        case pb.rpt.products.purchases.p.supplier:
          return tab_purchase_supplier(element, data, event);
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.products.purchases.p.category:
        return report_purchase_category(element, event);
      case pb.rpt.products.purchases.p.item:
        return report_purchase_item(element, event);
      case pb.rpt.products.purchases.p.supplier:
        return report_purchase_supplier(element, event);
      case pb.rpt.products.purchases.p.invoice:
