...
var PB_RC_sales = function () {
  ...
  const report_sale_direct = function (element, event) {
  ...
  const report_sale_group = function (element, event) {
  ...
  const report_sale_vehicle = function (element, event) {
  ...
  const tab_sale_direct = function (element, data) {
  ...
  const tab_sale_group = function (element, data, event) {
  ...
  const tab_sale_vehicle = function (element, data, event) {

    // value putting for category_id into customers.
    redData = PB_extend_foreign.$_remote('single', data, [['vehicles', 'customers', 2, 1]]);
    ...
    const _process_a = (data) => {
      ...
      switch (event.value) {
        case pb.rpt.customers.sales.p.direct:
          return tab_sale_direct(element, data, event);

        case pb.rpt.customers.sales.p.group:
          return tab_sale_group(element, data, event);

        case pb.rpt.customers.sales.p.vehicle:
          return tab_sale_vehicle(element, data, event);
      }
    }
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL(thePathArr, {
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.customers.sales.p.direct:
        return report_sale_direct(element, event);
      case pb.rpt.customers.sales.p.group:
        return report_sale_group(element, event);
      case pb.rpt.customers.sales.p.vehicle:
        return report_sale_vehicle(element, event);
