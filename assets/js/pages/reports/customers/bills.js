...
var PB_RC_bills = function () {
  ...
  const report_bill_view = function (element, event) {
  ...
  const report_bill_statement = function (element, event) {
  ...
  const report_bill_customer = function (element, event) {
  ...
  const tab_bill_view = function (element, data, button) {

    // value putting for category_id into customers.
    redData = PB_extend_foreign.$_remote('single', data, [['customers', 'bills', 4, 1]]);
  ...
  const tab_bill_statement = function (element, data, button) {


    // value putting for category_id into customers.
    redData = PB_extend_foreign.$_remote('single', data, [['months', 'years', 2, 1]]);
    // append options into selects.
    PB_render_form.renderME(rendER.form.method.selectOnly, redData, element, atr.core.source);
  ...
  const tab_bill_customer = function (element, data, button) {
    ...
    const _process_a = (data) => {
      ...
      switch (event.value) {
        case pb.rpt.customers.bills.p.view:
          return tab_bill_view(element, data, event);
        case pb.rpt.customers.bills.p.statement:
          return tab_bill_statement(element, data, event);
        case pb.rpt.customers.bills.p.customer:
          return tab_bill_customer(element, data, event);
      }
    }
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL(thePathArr, {
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.customers.bills.p.view:
        return report_bill_view(element, event);
      case pb.rpt.customers.bills.p.statement:
        return report_bill_statement(element, event);
      case pb.rpt.customers.bills.p.customer:
        return report_bill_customer(element, event);
