...
// Class definition
var PB_RC_transactions = function () {
  ...
  const report_transaction_simple = function (element, event) {
  ...
  const report_transaction_detailed = function (element, event) {
  ...
  const report_transaction_statement = function (element, event) {
  ...
  const tab_transaction_simple = function (element, data, event) {
  ...
  const tab_transaction_detailed = function (element, data, event) {
    PB_render_form.renderME(rendER.form.method.selectOnly, data, element, atr.core.source);
  ...
  const tab_transaction_statement = function (element, data, event) {
    // value putting for category_id into customers.
    redData = PB_extend_foreign.$_remote('single', data, [['customers', 'dues', 4, 1]]);
    ...
    const _process_a = (data) => {
      ...
      switch (event.value) {
        case pb.rpt.customers.transactions.p.simple:
          return tab_transaction_simple(element, data, event);
        case pb.rpt.customers.transactions.p.detailed:
          return tab_transaction_detailed(element, data, event);
        case pb.rpt.customers.transactions.p.statement:
          return tab_transaction_statement(element, data, event);
      }
    }
    const ajax = new AjaxPB();
    const urlID = ajax.buildingURL(thePathArr, {
  ...
  const reportsTarget = function (element, event) {
    switch (event.value) {
      case pb.rpt.customers.transactions.p.simple:
        return report_transaction_simple(element, event);
      case pb.rpt.customers.transactions.p.detailed:
        return report_transaction_detailed(element, event);
      case pb.rpt.customers.transactions.p.statement:
        return report_transaction_statement(element, event);
