...
var PB_MA_contacts = function () {
  ...
  const cardsTarget = function (element, event) {
    ...
    /**
     * it appends html table of contact list in target from DB.
     * @param element
     * @param event
     * @returns {{Promise}}
     */
    const card_list_contact = function (element, event) {
     ...
     */
    const card_edit_contact = function (element, event) {
     ...
     * @returns {AjaxPB}
     */
    const card_view_contact = function (element, event) {
      let contact_id = event.data;

      if (element.getAttribute(atr.core.loadOf.card) === '1') {
        if (element.getAttribute(atr.core.id) === contact_id) {
      ...
      const _process_a = (data) => {
        ...
        /*
         const navtab = pb.mng.accounts.contacts.c.tab.general;
        ...
        element.setAttribute(atr.core.id, contact_id)
     ...
     * @returns {boolean}
     */
    const card_add_contact = function (element, event) {
    ...
    switch (event.value) {
      ...
      case pb.mng.accounts.contacts.p.new:
        return card_add_contact(element, event);

      case pb.mng.accounts.contacts.p.edit:
        return card_edit_contact(element, event);

      case pb.mng.accounts.contacts.p.list:
        return card_list_contact(element, event);

      case pb.mng.accounts.contacts.p.view:
        return card_view_contact(element, event);
  ...
  const actionsTarget = function (element, button) {
     ...
     * @returns {boolean}
     */
    const action_save_contact = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        // it will reset the status if table is loaded and new contact has added.
     ...
     * @returns {boolean}
     */
    const action_edit_contact = function (element, event) {
      console.log(event);
     ...
     * @returns {boolean}
     */
    const action_delete_contact = function (element, event) {
    ...
    const action_disable_contact = (element, button) => {
    ...
    switch (button.value) {
      case pb.mng.accounts.contacts.c.form.delete:
        return action_delete_contact(element, button);

      case pb.mng.accounts.contacts.c.form.disable:
        return action_disable_contact(element, button);

      case pb.mng.accounts.contacts.c.form.save:
        return action_save_contact(element, button);

      case pb.mng.accounts.contacts.c.form.update:
        return action_edit_contact(element, button);
