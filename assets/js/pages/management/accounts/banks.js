...
var PB_MA_banks = function () {
  ...
  const cardsTarget = function (element, event) {
     ...
     */
    const card_list_bank = function (element, event) {
      console.log(event);
      ...
      const _process_a = (data) => {
        ...
        PB_extend_foreign.$_remote('double', [data.list, StateData.category], [[3, 'name']])
     ...
     */
    const card_edit_bank = function (element, event) {
     ...
     * @returns {AjaxPB}
     */
    const card_view_bank = function (element, event) {
      let bank_id = event.data;

      if (element.getAttribute(atr.core.loadOf.card) === '1') {
        if (element.getAttribute(atr.core.id) === bank_id) {
     ...
     * @returns {boolean}
     */
    const card_add_bank = function (element, event) {
    ...
    switch (event.value) {
      ...
      case pb.mng.accounts.banks.p.new:
        return card_add_bank(element, event);

      case pb.mng.accounts.banks.p.edit:
        return card_edit_bank(element, event);

      case pb.mng.accounts.banks.p.list:
        return card_list_bank(element, event);

      case pb.mng.accounts.banks.p.view:
        return card_view_bank(element, event);
  ...
  const navtabsTarget = function (_element, _event) {
    ...
    const navtab_deposits_view = function (element, data, event) {
      ...
      console.log(data)
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'deposit', 3, 0]]);
    ...
    const navtab_expenses_view = function (element, data, event) {
    ...
    const navtab_receipts_view = function (element, data, event) {
      ...
      // data foreign matching
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'transaction', 2, 0]]);
      // access table profile
      $table.matchingCalled(event.value)
    ...
    const _process_a = (data) => {
      ...
      switch (_event.value) {
        ...
        case pb.mng.accounts.banks.c.tab.expenses:
          return navtab_expenses_view(_element, data, _event);

        case pb.mng.accounts.banks.c.tab.deposits:
          return navtab_deposits_view(_element, data, _event);
  ...
  const actionsTarget = function (element, button) {
     ...
     * @returns {boolean}
     */
    const action_save_bank = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        if (!data.status) {
          toastr.success("Failed to create new bank", `Reason: ${data.message}`);
        }
        toastr.success('New Bank has been created', "Success");

        // it will reset the status if table is loaded and new bank has added.
    ...
    switch (button.value) {
      case pb.mng.accounts.banks.c.form.delete:
        return action_delete_bank(element, button);

      case pb.mng.accounts.banks.c.form.disable:
        return action_disable_bank(element, button);

      case pb.mng.accounts.banks.c.form.save:
        return action_save_bank(element, button);

      case pb.mng.accounts.banks.c.form.update:
        return action_edit_bank(element, button);
