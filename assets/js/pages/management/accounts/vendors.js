...
var PB_MA_vendors = function () {
  ...
  // Private functions
  function vendors_ajaxCalling(button, params = []) {
  ...
  const cardsTarget = function (element, event) {
     ...
     * @returns {Promise}
     */
    const card_list_vendor = function (element, event) {
    ...
    const card_edit_vendor = function (element, event) {
     ...
     * @returns {AjaxPB}
     */
    const card_view_vendor = function (element, event) {
      let vendor_id = event.data;

      if (element.getAttribute(atr.core.loadOf.card) === '1') {
        if (element.getAttribute(atr.core.id) === vendor_id) {
      ...
      const _process_a = (data) => {
        ...
        //        const navtab = pb.mng.accounts.vendors.c.tab.general;
        ...
        element.setAttribute(atr.core.id, vendor_id)
     ...
     * @returns {boolean}
     */
    const card_add_vendor = function (element, event) {
    ...
    switch (event.value) {
      ...
      case pb.mng.accounts.vendors.p.new:
        return card_add_vendor(element, event);

      case pb.mng.accounts.vendors.p.edit:
        return card_edit_vendor(element, event);

      case pb.mng.accounts.vendors.p.list:
        return card_list_vendor(element, event);

      case pb.mng.accounts.vendors.p.view:
        return card_view_vendor(element, event);
  ...
  const navtabsTarget = function (_element, _event) {
    ...
    const navtab_bills_view = function (element, data, event) {
      ...
      console.log(data);
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'ledgers', 3, 0], ['data', 'status', 5, 0]]);
    ...
    const navtab_dues_view = function (element, data, event) {
      ...
      // data foreign matching
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'date', 2, 0]]);
      // access table profile
      $table.matchingCalled(event.value);
    ...
    const _process_a = (data) => {
      ...
      switch (_event.value) {
        ...
        case pb.mng.accounts.vendors.c.tab.bills:
          return navtab_bills_view(_element, data, _event);
        ...
        case pb.mng.accounts.vendors.c.tab.dues:
          return navtab_dues_view(_element, data, _event);
  ...
  const actionsTarget = function (element, button) {
     ...
     * @returns {boolean}
     */
    const action_save_vendor = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        if (!data.status) {
          toastr.success("Failed to create new vendor", `Reason: ${data.message}`);
        }
        toastr.success('New vendor has been created', "Success");

        // it will reset the status if table is loaded and new vendor has added.
     ...
     * @returns {boolean}
     */
    const action_edit_vendor = function (element, event) {
      console.log(event);
    ...
    switch (button.value) {
      case pb.mng.accounts.vendors.c.form.delete:
        return action_delete_vendor(element, button);

      case pb.mng.accounts.vendors.c.form.disable:
        return action_disable_vendor(element, button);

      case pb.mng.accounts.vendors.c.form.save:
        return action_save_vendor(element, button);

      case pb.mng.accounts.vendors.c.form.update:
        return action_edit_vendor(element, button);
