...
var PB_MA_suppliers = function () {
  ...
  const cardsTarget = function (element, event) {
     ...
     * @returns {boolean}
     */
    const card_list_supplier = function (element, event) {
     ...
     * @returns {boolean}
     */
    const card_edit_supplier = function (element, event) {
     ...
     * @returns {AjaxPB}
     */
    const card_view_supplier = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        //        const navtab = pb.mng.accounts.suppliers.c.tab.general;
        ...
        element.setAttribute(atr.core.id, supplier_id)
     ...
     * @returns {boolean}
     */
    const card_add_supplier = function (element, event) {
    ...
    switch (event.value) {
      ...
      case pb.mng.accounts.suppliers.p.new:
        return card_add_supplier(element, event);

      case pb.mng.accounts.suppliers.p.edit:
        return card_edit_supplier(element, event);

      case pb.mng.accounts.suppliers.p.list:
        return card_list_supplier(element, event);

      case pb.mng.accounts.suppliers.p.view:
        return card_view_supplier(element, event);
  ...
  const navtabsTarget = function (_element, _event) {
    ...
    const navtab_purchases_view = function (element, data, event) {
      ...
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'category', 2, 0], ['data', 'status', 5, 0]]);
    ...
    const navtab_payments_view = function (element, data, event) {
      ...
      // data foreign matching
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'transfer_by', 2, 0]]);
      // access table profile
      $table.matchingCalled(event.value);
    ...
    const navtab_deductions_view = function (element, data, event) {
      ...
      // data foreign matching
      redData = PB_extend_foreign.$_remote(kws.extend.combine, data, [['data', 'date', 1, 0], ['data', 'account', 2, 0], ['data', 'transaction', 3, 0]]);
      // access table profile
      $table.matchingCalled(event.value);
    ...
    const navtab_incentives_view = function (element, data, event) {
    ...
    const _process_a = (data) => {
      ...
      switch (_event.value) {
        ...
        case pb.mng.accounts.suppliers.c.tab.purchases:
          return navtab_purchases_view(_element, data, _event);
        ...
        case pb.mng.accounts.suppliers.c.tab.deductions:
          return navtab_deductions_view(_element, data, _event);

        case pb.mng.accounts.suppliers.c.tab.incentives:
          return navtab_incentives_view(_element, data, _event);
  ...
  const actionsTarget = function (element, button) {
     ...
     * @returns {boolean}
     */
    const action_save_supplier = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        if (!data.status) {
          toastr.success("Failed to create new supplier", `Reason: ${data.message}`);
        }
        toastr.success('New supplier has been created', "Success");

        // it will reset the status if table is loaded and new supplier has added.
     ...
     * @returns {boolean}
     */
    const action_edit_supplier = function (element, event) {
      console.log(event);
     ...
     * @returns {boolean}
     */
    const action_delete_supplier = function (element, event) {
      ...
      if (!confirm('Are your sure for Delete this supplier ??')) {
    ...
    switch (button.value) {
      case pb.mng.accounts.suppliers.c.form.delete:
        return action_delete_supplier(element, button);
      ...
      case pb.mng.accounts.suppliers.c.form.save:
        return action_save_supplier(element, button);

      case pb.mng.accounts.suppliers.c.form.update:
        return action_edit_supplier(element, button);
