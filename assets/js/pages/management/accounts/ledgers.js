...
var PB_MA_ledgers = function () {
  ...
  // Private functions
  function ledgers_ajaxCalling(button, params = []) {
  ...
  const cardsTarget = function (element, event) {
     ...
     * @returns {boolean}
     */
    const card_list_ledger = function (element, event) {
     ...
     * @returns {boolean}
     */
    const card_edit_ledger = function (element, event) {
      ...
      ajax.callREQUEST({thePathArr}, urlID, false, _process);
      return ajax;
    }
    /**
     ...
     * @returns {AjaxPB}
     */
    const card_view_ledger = function (element, event) {
      ...
      const _process_a = (data) => {
        ...
        //        const navtab = pb.mng.accounts.ledgers.c.tab.general;
        ...
        element.setAttribute(atr.core.id, ledger_id)
     ...
     * @returns {boolean}
     */
    const card_add_ledger = function (element, event) {
    ...
    switch (event.value) {
      ...
      case pb.mng.accounts.ledgers.p.new:
        return card_add_ledger(element, event);

      case pb.mng.accounts.ledgers.p.edit:
        return card_edit_ledger(element, event);

      case pb.mng.accounts.ledgers.p.list:
        return card_list_ledger(element, event);

      case pb.mng.accounts.ledgers.p.view:
        return card_view_ledger(element, event);
  ...
  const actionsTarget = function (element, button) {
     ...
     * @returns {boolean}
     */
    const action_save_ledger = function (element, event) {
      ...
      const _process_a = (data) => {
        console.log(data);
        if (!data.status) {
          toastr.success("Failed to create new ledger", `Reason: ${data.message}`);
        }
        toastr.success('New ledger has been created', "Success");

        // it will reset the status if table is loaded and new ledger has added.
     ...
     * @returns {boolean}
     */
    const action_edit_ledger = function (element, event) {
      console.log(event);
     ...
     * @returns {boolean}
     */
    const action_delete_ledger = function (element, event) {
    ...
    const action_disable_ledger = (element, button) => {
    ...
    switch (button.value) {
      case pb.mng.accounts.ledgers.c.form.delete:
        return action_delete_ledger(element, button);

      case pb.mng.accounts.ledgers.c.form.disable:
        return action_disable_ledger(element, button);

      case pb.mng.accounts.ledgers.c.form.save:
        return action_save_ledger(element, button);

      case pb.mng.accounts.ledgers.c.form.update:
        return action_edit_ledger(element, button);
