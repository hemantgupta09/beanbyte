...
var PB_MC_listing = function () {
  ...
  function listing_ajaxCalling(button, params = []) {
    ...
    switch (btnValue) {
      ...
      case pb.mng.customers.listing.p.card:
        // call the ajax
        // var fetchData = JSON.parse(responseData["key"])
        response = {
          'rows'  :
        ...
        break;
      case pb.mng.customers.listing.p.table:
        // call the ajax
        // var fetchData = JSON.parse(responseData["key"])
        response = {
          'rows'  : [
  ...
  const pageOpen = () => {
    ...
    const _process_b = (data) => {
      ...
      //now load the customers number
      let tarEle = _listingLE.action.querySelector('h3>span');
      tarEle.innerHTML = Custmors;
  ...
  const loadCards = function (element, event) {
    ...
    if (place.getAttribute('data-pb-cards-loaded') === "1") {
      return true;
    }

    const _process = (data) => {
      _process_a(data);
      return _process_b(data.customers);
    }

    const _process_a = (data) => {
      PB_extend_foreign.$_remote('single', data, [
        [
          "customers", "dates", "overdue", 0
        ],
        [
          "customers", "category", "category_id", 0
        ]
      ]);
    }
    const _process_b = (data) => {
      ...
      Object.keys(data).forEach((key, index) => {
        ...
        Object.keys(nodeData.tags).forEach((tag, index) => {
          if (!nodeData.tags[tag]) {
            tags.querySelector('span[name="' + tag + '"]').classList.add('d-none');
          }
        })
        ...
        place.setAttribute('data-pb-cards-loaded', '1');
      });
      return true;
    }
  ...
  const loadElement = function (element) {
    ...
    for (let i = 1; i <= Math.ceil(Custmors / 20); i++) {
    ...
    //make paging button
    place = element.querySelector('[data-pb-target="pagination"]');

    let arrList = Array.from(place.children);

    for (let i = 2; i <= Math.ceil(Custmors / 20); i++) {
   ...
   * @param button
   */
  const loadPaging = function (element, event) {

    let tarEle = element.querySelector('[data-pb-target="pagination"]');
    ...
    tarEle.children.forEach((ele, index) => {
      ...
      if (ele.classList.contains('active')) {
        ...
        if (!tarEle.children[index + 1].classList.contains('next')) {
          tarEle.children[index + 1].classList.add('d-none');
        }
      ...
      if (ele.children[0].value === event.value) {
        ...
        tarEle.children[index + 1].classList.remove('d-none');
        tarEle.children[index - 1].classList.remove('d-none');
   ...
   * @returns {boolean}
   */
  const loadTables = function (element, button) {
    tableEle = element.querySelector('table');
    filterEle = element.querySelector('[data-pb-target="filter"]');

    if (tableEle.getAttribute('data-pb-table-loaded') === "1") {
      return true;
    }
    const ajax = new AjaxPB();
   ...
   * @param button
   */
  const loadNavTab = function (element, event) {
    switch (event.value) {
      case pb.mng.customers.listing.p.card:
        return loadPaging(element, {
          value: "1",
          type : "paging",
        });
      case pb.mng.customers.listing.p.table:
        return loadTables(element, event);
    }
  }
  ...
  return {
    ...
    // loading form
    MCL_action : function (path) {
      return true;
    },
    MCL_navtab : function (_event) {
      return eleCheck() ? loadNavTab(_callEl, _event) : false;
    },
    MCL_paging : function (_event) {
      return eleCheck() ? loadPaging(_callEl, _event) : false;
    },
    MCA_creates: function (path) {
      return true;
    },
