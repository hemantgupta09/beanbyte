...
var PB_FB_summary = function () {
    ...
    const $head = () => {
      ...
      for (const masterKey in data.head) {
        Object.keys(data.head[masterKey]).forEach((headKey) => {
          ...
          headNode.querySelector('[' + atr.core.put + '="totalbalgroup"]').innerText = row[2];
    ...
    const $group = () => {
      ...
      for (const headKey in data.group) {
        Object.keys(data.group[headKey]).forEach((groupKey) => {
          ...
          groupNode.querySelectorAll('button').forEach((btn) => {
  ...
  const Sheet = (element, event) => {
    ...
    const sheetData = dummy_ajax_balances_fin.summary([pb.fin.balance.summary.p.sheet]);
  ...
  const Table = () => {
    ...
    const tableData = dummy_ajax_balances_fin.summary([pb.fin.balance.summary.p.trans]);
