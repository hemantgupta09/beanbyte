...
var PB_FB_comparison = function () {
    ...
    const $ledger = () => {
      ...
      for (const groupKey in data.ledger) {
        Object.keys(data.ledger[groupKey]).forEach((ledgerKey) => {
          ...
          ledgerNode.querySelector('[' + atr.core.put + '="balancepy"]').innerText = row[2];
          ledgerNode.querySelector('[' + atr.core.put + '="balancecy"]').innerText = row[3];
  ...
  const Sheet = (element, event) => {
    ...
    const sheetData = dummy_ajax_balances_fin.comparison([pb.fin.balance.comparison.p.sheet]);
  ...
  const Table = () => {
    ...
    const tableData = dummy_ajax_balances_fin.comparison([pb.fin.balance.comparison.p.trans]);
