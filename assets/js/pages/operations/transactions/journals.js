...
var PB_OT_journals = function () {
  ...
  const pageOpen = () => {
    ...
    const _process_b = (data) => {
      ...
      PB_render_menu.menuRender(xData.menu, 'value');
    }
    ...
    //    xResponse = dummy_ajax_transactions_opr.journals([pb.opr.transactions.journals.n, 'page']);
    return ajax;
  }
  ...
  function createsTarget(element, event) {
    ...
    const _process_a = (data) => {
      ...
      xData = data.data;
      const sts = PB_render_form.renderME(rendER.form.method.repeater, {...StateData, ...xData}, formEle);
    ...
    if (formEle.getAttribute(atr.core.loadOf.form) !== '1') {
      ...
      //      xResponse = dummy_ajax_transactions_opr.journals([event.value, event.type], [])
    }
  }
  ...
  function loadsTarget(element, button) {
    ...
    const getLoad = function (form, button, values) {
      ...
      //      xResponse = dummy_ajax_transactions_opr.journals([button.value, button.getAttribute('name')], values)
    ...
    //      child.innerHTML = PB_extend_append.collectAppend(child.innerHTML, data[key]);
  ...
  function tablesTarget(element, button) {
    const getTable = (button, params, htLen) => {
      ...
      const _process_a = (data) => {
        if (data !== false) {
          const OPTION = [4, ['view', 'edit', 'delete'], ['view-journals', 'edit-journals', 'delete-journals']];
          let tableProfile = {
            0: {
              type  : TBL_ct.cb,
              method: 'a',
      ...
      //      xResponse = dummy_ajax_transactions_opr.journals([button.value, button.getAttribute('name')], params)
