...
var PB_OT_payments = function () {
  ...
  const cardsTarget = function (element, event) {
    ...
    const _card_modal = (element, event) => {
      ...
      const _process = (data) => {
        _process_card_a(data);
      }
      const _process_card_a = (data) => {
  ...
  function tablesTarget(element, button) {
    const getTable = (button, params, htLen) => {
      ...
      const _process = (data) => {
        _process_table_a(data);
      }
      const _process_table_a = (data) => {
        if (data !== false) {
          const OPTION = [4, ['view', 'edit', 'delete'], ['view-payments', 'edit-payments', 'delete-payments']];
          let tableProfile = {
            0: {
              type  : TBL_ct.cb,
              method: 'a',
      ...
      //      xResponse = dummy_ajax_transactions_opr.payments([button.value, button.getAttribute('name')], params)
