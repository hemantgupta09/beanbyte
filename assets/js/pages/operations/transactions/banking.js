...
var PB_OT_banking = function () {
  ...
  const cardsTarget = function (element, event) {
    ...
    const _process_a = (data) => {
      //      const a = element.querySelector('[data-pb-control="delete-entry"]').
      //      console.log(a);
      ...
      for (var i = 0, row; row = table.rows[i]; i++) {
        for (var j = 0, col; col = row.cells[j]; j++) {
          ...
          col.innerHTML = PB_extend_append.$_single(col.innerHTML, data[key]);
          PB_render_common.initOnCall(col);
        }
      }
    ...
    //    const b = a.getAttribute(atr.core.key);
    //    console.log(a);
  ...
  function loadsTarget(element, button) {
    ...
    const getLoad = function (form, button, values) {
      ...
      //      xResponse = dummy_ajax_transactions_opr.banking([button.value, button.getAttribute('name')], values)
  ...
  function tablesTarget(element, button) {
    const getTable = (button, params, htLen) => {
      ...
      const _process = (data) => {
        return _process_table_a(data);
      }
      const _process_table_a = (data) => {
        if (data !== false) {
      ...
      //      xResponse = dummy_ajax_transactions_opr.banking([button.value, button.getAttribute('name')], params)
...
/*
 [] on the click of back, reset the form.
 */